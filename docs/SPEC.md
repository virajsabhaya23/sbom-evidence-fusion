...
a conflict; weak-only disagreement remains explicitly qualified evidence. The
machine-readable `weak_hash_policy` is `record-only-no-identity-conflict`:
weak values and their source ledger are retained, but they can neither confirm
nor conflict material identity.
